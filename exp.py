...
class States:
    ...
    def update(self, sim, event):
        ...
        '''
        print('simulation clock  now     %lf '%(sim.simclock))
        print('time of   last event      %lf '%(self.time_last_event))
        print('time since last event     %lf \n'%(time_since_last_event))
        '''
        self.time_last_event=sim.simclock
        self.area_num_in_q =self.area_num_in_q +(self.num_in_q_in_last_event*time_since_last_event)
        '''
        print('queue te ekhn  manus ache    %lf '%(sim.num_in_q))
        print('queue te age   manus chilo    %lf '%(self.num_in_q_in_last_event))
        print('area add hoeche current event er jonno %lf '%(self.num_in_q_in_last_event*time_since_last_event))
        print('total area for num in queue   %lf \n\n\n\n\n\n\n\n'%(self.area_num_in_q))
        '''
        self.area_server_status+=self.last_server_status*time_since_last_event
    ...
    def finish(self, sim):
        ...
        '''
        print('final simulation clock %lf'%(sim.simclock))
        print('final area in queue  %lf'%(self.area_num_in_q))
        print('final total_of_delays    %lf'%(self.total_of_delays))
...
class StartEvent(Event):
    ...
    def process(self, sim):
        ...
        print("total server no %lf  "%(sim.states.available_server))
...
class ArrivalEvent(Event):
    ...
    def process(self, sim):
        ...
        if sim.states.server_status==1:
           ...
           if sim.num_in_q>10:
              sim.scheduleEvent(ExitEvent(sim.simclock+50,sim))
            
        elif sim.states.server_status==0:
             sim.states.served+=1
...
def experiment1(mu,lm,k):
    seed = 101
    sim = Simulator(seed)
   # sim.configure(Params(5.0 / 60, 8.0 / 60, 1), States())
    sim.configure(Params(mu,lm,k), States())
